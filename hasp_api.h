...
/*! \brief Configure the API setting some options.

 *This function deconfigure the API cleaning all the resources and restore it to the initial state. 
 *This API already exists, but it's not documented. It was shared to some vendor, and then it cannot be changed.
 *
 *hasp_status_t hasp_cleanup(void);
 *
 * \param config Different XML elements, that allow to configure different kind of options
 *
 * \return            <ul>
 *                      <li>HASP_STATUS_OK</li> All options are applied
 *                      <li>HASP_INV_SETUP</li> At least one option was invalid (others were applied)
 *                      <li>HASP_INV_ORDER</li> At least one option was called with a wrong order. Like options that can be set only at startup.
 *                    </ul>
 *
 */
